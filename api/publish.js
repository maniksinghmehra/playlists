...
module.exports = async (req, res) => {
  ...
  try {
    ...
    if (!expectedPassword) {
      res.status(500).json({ error: "The pass environment variable is not configured for this Vercel deployment." });
      return;
    }
    if (normalizePassword(adminPassword) !== normalizePassword(expectedPassword)) {
